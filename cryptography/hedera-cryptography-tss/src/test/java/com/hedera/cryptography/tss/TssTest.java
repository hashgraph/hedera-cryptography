...
class TssTest {
    ...
    void testGenesis() {
        ...
        final TssMessage p0Message = tssService.getGenesisStage().generateTssMessage(p0sDirectory);
        ...
        // this message will contain a random share split in 3 parts
        final TssMessage p1Message = tssService.getGenesisStage().generateTssMessage(p1sDirectory);
        ...
        // this message will contain a random share split in 3 parts
        final TssMessage p2Message = tssService.getGenesisStage().generateTssMessage(p2sDirectory);
        ...
        final List<TssMessage> validMessages = messages.stream()
                .filter(tssMessage -> tssService.getGenesisStage().verifyTssMessage(p0sDirectory, tssMessage))
                .toList();
        ...
        final List<TssPrivateShare> privateShares = Objects.requireNonNull(
                tssService.getGenesisStage().obtainPrivateShares(p0sDirectory, validMessages),
        ...
        final List<TssPublicShare> publicShares = Objects.requireNonNull(
                tssService.getGenesisStage().obtainPublicShares(p0sDirectory, validMessages),
                "Condition of threshold number of messages was not met");
    ...
    void testSigning() {
        ...
        final var validMessages = tssMessages.stream()
                .filter(m -> tssService.getGenesisStage().verifyTssMessage(p0sDirectory, m))
                .toList();
        final var publicShares =
                tssService.getRekeyStage().obtainPublicShares(p0sDirectory, validMessages); // Reduce to threshold?

        final var ledgerID = TssPublicShare.aggregate(publicShares);

        final byte[] messageToSign = new byte[20];
        rng.nextBytes(messageToSign);
        final var privateShares = tssService.getRekeyStage().obtainPrivateShares(p0sDirectory, validMessages);
    ...
    void rekeying() {
        ...
        final var oldPublicShares =
                tssService.getGenesisStage().obtainPublicShares(p0sDirectory, validMessages); // Reduce to threshold?
        final var oldP0PrivateShares =
                tssService.getGenesisStage().obtainPrivateShares(p0sDirectory, validMessages); // Reduce to threshold?

        final var ledgerID = TssPublicShare.aggregate(oldPublicShares);
        // then:
        final List<TssMessage> p0Messages = oldP0PrivateShares.stream()
                .map(p -> tssService.getRekeyStage().generateTssMessage(p0sDirectory, p))
                .toList();
        ...
        final List<TssMessage> collectedValidMessages = Stream.of(p0Messages, p1Messages, p2Messages)
                ...
                .filter(tssMessage ->
                        tssService.getRekeyStage().verifyTssMessage(p0sDirectory, oldPublicShares, tssMessage))
                .toList();

        // Get the list of PrivateShares owned by participant 0
        final List<TssPrivateShare> newP0privateShares = Objects.requireNonNull(
                tssService.getRekeyStage().obtainPrivateShares(p0sDirectory, collectedValidMessages),
                "Condition of threshold number of messages was not met");

        // Get the list of PublicShares
        final List<TssPublicShare> newPublicShares = Objects.requireNonNull(
                tssService.getRekeyStage().obtainPublicShares(p0sDirectory, collectedValidMessages),
                "Condition of threshold number of messages was not met");
