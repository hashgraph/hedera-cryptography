...
import com.hedera.cryptography.tss.impl.ShamirUtils;
...
import com.hedera.cryptography.tss.impl.nizk.NizkWitness;
...
public abstract class Groth21Stage {
     ...
     */
    @NonNull
    protected Groth21Message fromTssMessage(@NonNull final TssMessage tssMessage) {
        if (!(tssMessage instanceof Groth21Message)) {
            final CipherText[] cipherTexts =
                    new CipherText[tssMessage.shareCiphertexts().size()];
            for (int i = 0; i < cipherTexts.length; i++) {
                cipherTexts[i] = new CipherText(tssMessage.shareCiphertexts().get(i));
            }
            final CiphertextTable combinedCipherText = new CiphertextTable(tssMessage.sharedRandomness(), cipherTexts);
            final NizkProof nizkProof =
                    new NizkProof(tssMessage.f(), tssMessage.a(), tssMessage.y(), tssMessage.zR(), tssMessage.zA());
            return new Groth21Message(
                    TssMessage.MESSAGE_CURRENT_VERSION,
                    signatureSchema,
                    tssMessage.generatingShare(),
                    combinedCipherText,
                    new EcPolynomial(tssMessage.polynomialCommitments()),
                    nizkProof);
        }
     ...
     */
    protected List<Groth21Message> fromTssMessages(@NonNull final List<TssMessage> tssMessages) {
        return Objects.requireNonNull(tssMessages, "tssMessages must not be null").stream()
                .map(this::fromTssMessage)
                .toList();
    }
