...
pub fn field_elements_zero<F:Field>() -> F {
    F::zero()
}

pub fn field_elements_one<F:Field>() -> F {
    F::one()
}

pub fn field_elements_multiply<F:Field>(value:F, value2:F) -> F {
    value * value2
}

pub fn field_elements_add<F:Field>(value:F, value2:F) -> F {
    value + value2
}

pub fn field_elements_subtract<F:Field>(value:F, value2:F) -> F {
    value - value2
}

pub fn field_elements_from_random<F:Field>() -> F {
    let mut rng = rand_chacha::ChaCha8Rng::from_seed([0u8; 32]);
    F::rand(&mut rng)
}

pub fn field_elements_from_int<F:Field>(value:u64) -> F {
    F::from(value)
}
...
pub fn scalars_minus<G: CurveGroup>(value: ScalarField<G>, value2: ScalarField<G>) -> ScalarField<G>{
    value - value2
}

pub fn scalars_multiply<G: CurveGroup>(value: ScalarField<G>, value2: ScalarField<G>) -> ScalarField<G>{
    value * value2
}
...
pub fn scalars_multiple_from_random<G: CurveGroup>() -> Vec<ScalarField<G>>{
    let mut rng = rand_chacha::ChaCha8Rng::from_seed([0u8; 32]);
    (0..32).map(|_| ScalarField::<G>::rand(&mut rng)).collect::<Vec<ScalarField<G>>>()
}
...
pub fn scalars_accum<G: CurveGroup>(values: Vec<ScalarField<G>>) -> ScalarField<G>{
    values.iter()
        ...
        .fold(
            ScalarField::<G>::zero(),
            |acc, (i, &r)| acc + r * ScalarField::<G>::from(256u64).pow([i as u64])
        )
}
...
pub fn  group_elements_scalar_multiply<G:CurveGroup>(value:G, value2: ScalarField<G>) ->G::Affine{
...
pub fn group_elements_g1_from_xy_bytes(
    x:&[u8],
    y:&[u8]
) -> ark_bn254::G1Projective {
    ark_bn254::G1Projective::from(G1Affine::new_unchecked( //if the coordinates are trusted
