...
import static com.hedera.cryptography.ceremony.crypto.KeyCertPurpose.AGREEMENT;
import static com.hedera.cryptography.ceremony.crypto.KeyCertPurpose.SIGNING;
...
public record PublicStores(KeyStore sigTrustStore, KeyStore agrTrustStore) {
     ...
     * 		if there is no provider that supports KEYSTORE_TYPE
    ...
    public X509Certificate getCertificate(final KeyCertPurpose type, final NodeId nodeId) throws KeyLoadingException {
        ...
        try {
            ...
            throw new KeyLoadingException("KeyStoreException", e);
        }
        if (certificate == null) {
