...
open class CargoBuildTask : DefaultTask() {
    @Input var toolchain: Toolchain? = null

    @Suppress("unused")
    @TaskAction
    fun build() =
        with(project) {
            extensions.getByType(CargoExtension::class.java).apply {
                ...
                // CARGO_TARGET_DIR can be used to force the use of a global, shared target
                // directory
                ...
                // something you should ever need to do currently, but we don't want it to ruin
                // anyone's
                ...
                copy {
                    ...
                    if (targetIncludes != null) {
                        ...
                        // It's safe to unwrap, since we bailed at configuration time if this is
                        // unset.
                        val libname = libname!!
    ...
    private fun buildProjectForTarget(
        project: Project,
        toolchain: Toolchain,
                ...
                project.properties.forEach { (key, value) ->
                    if (key.startsWith(prefix)) {
                        ...
                        project.logger.debug(
                            "Passing through environment variable '${key}' as '${realKey}=${value}'"
                        )
                        environment(realKey, value)
                    }
                }

                cargoExtension.extraCargoBuildArguments?.let { theCommandLine.addAll(it) }
